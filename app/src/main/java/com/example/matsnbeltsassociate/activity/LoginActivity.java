...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    private EditText mServiceAreaView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMobileView = findViewById(R.id.mobileno);
...
//        });
    ...
    private void attemptLogin() {
        ...
        mServiceAreaView.setError(null);
...
//            intent.putExtra(EXTRA_MESSAGE, message);
            ...
            verifyUserInDB(email, password, mobile, serviceArea);
        }
    }

    private void verifyUserInDB(final String email, final String pwd, final String mobile, final String serviceArea) {
        DatabaseReference m_supplierAccountsListRef = FirebaseDatabase.getInstance().getReference("Associate").child(mobile);
        ...
        m_supplierAccountsListRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while(itr.hasNext()) {
                    ...
                    String value = i.getValue(String.class);
                    if(key.equals("email") && value.equalsIgnoreCase(email)) {
                        emailVerify = true;
                    }
                    if(key.equals("password") && value.equalsIgnoreCase(pwd)) {
                        pwdVerify = true;
                    }

                }
                if ( emailVerify && pwdVerify ) {
                    showProgress(true);
    ...
    private void createDB(String email, String pwd, String mobile, String serviceArea) {
        DatabaseReference m_supplierAccountsListRef = FirebaseDatabase.getInstance().getReference("Associate");
        //String key = m_supplierAccountsListRef.push().getKey();
        String key = mobile;
        m_supplierAccountsListRef.child(key).child("email").setValue(email);
        m_supplierAccountsListRef.child(key).child("password").setValue(pwd);
        m_supplierAccountsListRef.child(key).child("mobile").setValue(mobile);
        m_supplierAccountsListRef.child(key).child("serviceArea").setValue(serviceArea);
        Intent intent = new Intent(getApplicationContext(), MainActivity.class);
        intent.putExtra(LauncherActivity.EXTRA_MESSAGE, mobile);
        writeUserNametoLocalFile(mobile);
        startActivity(intent);
